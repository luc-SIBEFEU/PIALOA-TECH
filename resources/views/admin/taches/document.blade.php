...
@section('title', 'Document - ' . $tache->nom)

@section('content')
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold">Document de {{ $tache->nom }}</h1>
            <p class="text-slate text-sm">{{ $nomFichier }} ({{ $mimeType }})</p>
        </div>
        <div class="space-x-3">
            <a href="{{ route('admin.stagiaires.download', $tache) }}" 
    ...
    <div class="mt-4 flex justify-end space-x-3">
        <form action="{{ route('admin.stagiaires.document.delete', $tache) }}" 
              method="POST" 
